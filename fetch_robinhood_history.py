"""Fetch the full daily history for EVERY equity symbol from Robinhood into SQLite.
...
rights, warrants - anything with a quote, excluding crypto) and writes rows
directly into data/robinhood.sqlite3 (bars table, upsert). No txt files.
The symbol list is saved to the symbols table in the robinhood sqlite.
...
Symbols already in the DB are skipped (--refresh to refetch), so reruns
...
Usage:
    ...
    python fetch_robinhood_history.py --refresh     # refetch existing symbols
...
def main() -> int:
    ...
    if args.dry_run:
        ...
        for sym in todo[:20]:
            rc.log(log_fh, f"  {sym:12s} -> data/robinhood.sqlite3")
        if len(todo) > 20:
